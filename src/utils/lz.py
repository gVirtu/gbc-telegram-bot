...
class Compressed:
    ...
    def score(self):
        ...
        for method in ['iterate', 'alternate', 'blank']:
            self.score_literal(method)
    ...
    def do_winner(self):
        winners = [
            (method, score) for method, score in self.scores.items()
            if score > self.min_scores[method] + int(score > lowmax)
        ]
        winners.sort(
            key=lambda x: (
                -(x[1] - self.min_scores[x[0]] - int(x[1] > lowmax)),
                self.preference.index(x[0])
            )
        )
