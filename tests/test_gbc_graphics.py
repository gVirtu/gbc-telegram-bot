...
class TestDecode1bpp:
    def test_all_index_1(self):
        data = bytes([0xFF] * 8)
        result = decode_1bpp(data, width=8, height=8)
        assert result[0] == [1] * 8
        assert result[7] == [1] * 8
    ...
    def test_two_tiles_wide(self):
        tile0 = bytes([0xFF] * 8)
        tile1 = bytes([0x00] * 8)
        result = decode_1bpp(tile0 + tile1, width=16, height=8)
        assert result[0] == [1] * 8 + [0] * 8

    def test_output_dimensions(self):
        data = bytes(64)
        result = decode_1bpp(data, width=16, height=32)
