...
class TestUpdateGroupAvatarCleanup:
    """Test that _update_group_avatar calls cleanup_after_avatar_update correctly."""
    ...
    @pytest.mark.asyncio
    async def test_cleanup_called_after_successful_photo_update(self, handler):
        """cleanup_after_avatar_update is called after a successful avatar update."""
        mock_adapter = MagicMock()
        mock_adapter.update_chat_photo = AsyncMock()
        mock_adapter.cleanup_after_avatar_update = AsyncMock()

        mock_controller = MagicMock()
        mock_png = MagicMock()
        mock_png.getvalue.return_value = b"png"
        mock_controller.get_frame_as_png.return_value = mock_png

        mock_config = MagicMock()
        mock_config.last_avatar_update_at = None

        fresh_config = MagicMock()
        fresh_config.latest_telegram_message_id = 200

        with patch("src.handlers.input_handler.state_manager") as mock_sm:
            mock_sm.load_chat_config.return_value = fresh_config
            mock_sm.save_chat_config = MagicMock()
            await handler._update_group_avatar(
                chat_id=111,
        ...
        mock_adapter.cleanup_after_avatar_update.assert_called_once_with(111, 200)

    @pytest.mark.asyncio
    async def test_cleanup_not_called_when_photo_update_raises(self, handler):
        """cleanup_after_avatar_update is NOT called when update_chat_photo raises."""
        mock_adapter = MagicMock()
        mock_adapter.update_chat_photo = AsyncMock(side_effect=Exception("failed"))
        mock_adapter.cleanup_after_avatar_update = AsyncMock()
        ...
        with patch("src.handlers.input_handler.state_manager"):
            with patch("src.handlers.input_handler.translation_manager"):
                await handler._update_group_avatar(
                    chat_id=111,
                    controller=mock_controller,
                    adapter=mock_adapter,
        ...
        mock_adapter.cleanup_after_avatar_update.assert_not_called()
