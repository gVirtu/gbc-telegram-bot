...
async def test_maintenance_command_shows_status_when_no_args(mock_update, mock_context):
    ...
    with patch('src.handlers.commands._check_chat_allowed', return_value=True):
        with patch('src.handlers.commands._check_admin_permission', return_value=(True, None)):
...
async def test_maintenance_command_rejects_non_admin(mock_update, mock_context):
    ...
    with patch('src.handlers.commands._check_chat_allowed', return_value=True):
        with patch('src.handlers.commands._check_admin_permission', return_value=(False, "Admin only")):
            await maintenance_command(mock_update, mock_context)
